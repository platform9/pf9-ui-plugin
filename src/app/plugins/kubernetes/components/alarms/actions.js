...
export const loadAlerts = createContextLoader(
  ...
  async ({ clusterId }, loadFromContext) => {
    ...
    if (clusterId === allKey) {
      // const all = await someAsync(pluck('uuid', clusters).map(qbert.getPrometheusAlerts)).then(
      //   flatten,
      // )
      const all = await someAsync(pluck('uuid', clusters).map(qbert.getPrometheusAlerts)).then(
        flatten,
