...
class ClusterDetachNodeDialog extends React.PureComponent {
  ...
  handleSubmit = async () => {
    ...
    const nodeUuids = Object.keys(this.state).filter((uuid) => this.state[uuid] === 'detach')

    const clusterUuid = row.uuid
    await clusterActions.detachNodesFromCluster({
      getContext,
      setContext,
      params: { clusterUuid, nodeUuids },
    })
    this.handleClose()
  }
    ...
    return (
      <Dialog open onClose={this.handleClose} onClick={stopPropagation}>
        ...
        <DialogContent>
          ...
          <ul>
            <li>All containers running on the node will be destroyed.</li>
            <li>
              Deployment will be automatically restarted by Kubernetes on another node in the
              cluster provided sufficient resources are available.
            </li>
            <li>
              This does not uninstall any Kubernetes packages from the node. Hence the node is
              available for reattaching to the same or different cluster.
            </li>
          </ul>
