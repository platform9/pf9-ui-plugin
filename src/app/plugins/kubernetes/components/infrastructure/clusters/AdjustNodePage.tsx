...
import {
  validators,
  minWorkerNodeValidator,
  maxWorkerNodeValidator,
} from 'core/utils/fieldValidators'
import { ICluster } from './model'
import { FormFieldCard } from 'core/components/validatedForm/FormFieldCard'
import { routes } from 'core/utils/routes'
import { CloudProvidersFriendlyName, CloudProviders } from '../cloudProviders/model'
...
const canScaleWorkers = (cluster) =>
  cluster.taskStatus === 'success' && cluster.cloudProviderType !== CloudProviders.Azure
...
const scaleConstraints: IConstraint[] = [
  // shrink
  {
    ...
    relation: 'deny',
    message:
      'You cannot remove master node from a single master cluster. If you wish to delete ' +
  ...
  },
  {
    ...
    relation: 'warn',
    message:
      'Removing this master node will reduce the total number of masters in this cluster ' +
  ...
  },
  {
    ...
    relation: 'warn',
    message:
      'For high availability, we recommend having at least 3 masters in a cluster at any ' +
  ...
  { startNum: 4, desiredNum: 3, relation: 'allow', message: '' },
  {
    ...
    relation: 'warn',
    message:
      'Removing this master node will result in an even number of master nodes for this ' +
  ...
  // grow
  {
    ...
    relation: 'deny',
    message:
      'You cannot add master nodes to a single master cluster.  You need to create a ' +
  ...
  { startNum: 2, desiredNum: 3, relation: 'allow', message: '' },
  {
    ...
    relation: 'warn',
    message:
      'Adding this master node will result in an even number of master nodes for this ' +
...
// Backend returns integer 0 and 1 instead of true and false
const isMaster = (node) => node.isMaster === 1

const ScaleNodeListPicker: React.FC<IScaleNodeListPickerProps> = ({
  scaleType,
  nodeType,
  cluster,
  transition,
  ...
  const getNodeFilter =
    scaleType === 'up' ? isUnassignedNode : allPass([isNotMaster, inCluster(cluster.uuid)])
  ...
  if (nodeType === 'worker') {
    return (
      <>
            ...
            <div className={classes.inputWidth}>
              {!cluster.enableCAS && (
                <TextField
                  ...
                  required
                  validations={[
                    validatorsAny.rangeValue(
                      calcMin(cluster.numWorkers, scaleType),
                      calcMax(cluster.numWorkers, scaleType),
                    ),
                  ]}
                />
              )}
              {!!cluster.enableCAS && (
                <>
                  <TextField
                    ...
                    validations={[
                      validatorsAny.rangeValue(
                        calcMin(cluster.numMinWorkers, scaleType),
                        calcMax(cluster.numMinWorkers, scaleType),
                      ),
                    ]}
                    required
                  />
                  <TextField
                    ...
                    validations={[
                      validatorsAny.rangeValue(
                        calcMin(cluster.numMaxWorkers, scaleType),
                        calcMax(cluster.numMaxWorkers, scaleType),
                      ),
                    ]}
                    required
                  />
        ...
        {canScaleWorker && relation === 'warn' && (
          <Alert small message={message} variant="warning" />
        )}
        {canScaleWorker && (
          <div className={classes.tableWidth}>
            <ClusterHostChooser
              ...
              multiple
              pollForNodes
              filterFn={getNodeFilter}
              validations={minMaxValidator}
            />
          </div>
        )}
      </>
  ...
  if (!canScaleMasters(cluster)) {
    return (
      <Alert
        small
        message="Scale master operation is not valid for this cluster. Please check your cluster status and the number of master nodes are valid"
        variant="error"
      />
    )
  }
...
const ScaleTypeRadioGroup: React.FC<IScaleTypeRadioGroupProps> = ({
  scaleType,
  handleScaleTypeChange,
    ...
    <>
      ...
      <RadioGroup
        aria-label="scaleType"
        name="scaleType"
        row
        value={scaleType}
        onChange={handleScaleTypeChange('scaleType')}
      >
        <FormControlLabel
          value="up"
...
const NodeTypeRadioGroup: React.FC<INodeTypeRadioGroupProps> = ({
  nodeType,
...
const AdjustNodePage: React.FC<{}> = () => {
  ...
  }, [isAttaching, isDetaching, isUpdating])
  ...
  const handleChange = useCallback((key) => (e) => updateParams({ [key]: e.target.value }), [
    params.nodeType || params.scaleType,
  ])

  const canSubmit =
    nodeType === 'master'
      ? canScaleMasters(cluster) && transition.relation !== 'deny'
      : canScaleWorkers(cluster)

  return (
    <FormWrapper
      className=""
      title="Adjust node for cluster"
      backUrl={listUrl}
      loading={isLoading}
      message={getLoadingMessage()}
    >
      <ValidatedForm initialValues={params} onSubmit={handleSubmit}>
        <div className={classes.formWidth}>
          ...
          <div className={classes.submit}>
            {canSubmit && (
              <SubmitButton>{SubmitButtonLabel[`${scaleType}-${nodeType}`]}</SubmitButton>
            )}
          </div>
        </div>
      </ValidatedForm>
