...
const AwsCloudProviderRequirementDialog = ({ showDialog, setShowDialog }) => {
  const classes = useStyles()
  const { history } = useReactRouter()

  const handleCloudProviderNavigation = () =>
    history.push(routes.cloudProviders.add.path({ type: CloudProviders.Aws }))
    ...
    <Dialog maxWidth="md" open={showDialog}>
      ...
      >
        <Text variant="body1" className={classes.text}>
          Connect your AWS account by creating a AWS Cloud Provider.
        </Text>
        <Text variant="body1" className={classes.text}>
          Please note that a Route 53 domain name registration will also be required which can be
          designated during the creation of the new AWS Cloud Provider.
        </Text>
      </FormFieldCard>
...
export default AwsCloudProviderRequirementDialog
