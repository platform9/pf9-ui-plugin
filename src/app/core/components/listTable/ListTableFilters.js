...
export const styles = {
  ...
  filters: {
    ...
    height: '80%',
    justifyContent: 'space-between',
    flexWrap: ({ inline }) => (inline ? 'nowrap' : 'wrap'),
  },
  formGroup: {
...
class ListTableFilters extends React.PureComponent {
  renderWrappedFilter = (filter) => (
    <FormGroup key={filter.columnId} className={this.props.classes.formGroup}>
  ...
  render() {
    ...
    const filtersDiv = (
      <div className={classes.filters}>{filters.map(this.renderWrappedFilter)}</div>
    )
    if (inline) {
      return filtersDiv
    }
...
export default compose(withStyles(styles))(ListTableFilters)
