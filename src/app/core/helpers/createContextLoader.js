import {
  pick, identity, assoc, find, whereEq, when, isNil, reject, filter, pipe, lensPath, pickAll, view,
  has, equals, values, either, sortBy, reverse, mergeLeft, map, toLower, is, head, prop,
} from 'ramda'
    ...
    async (params = emptyObj, refetch = contextLoaderFn[invalidateCacheSymbol], additionalOptions = emptyObj) => {
      ...
      try {
        ...
        if (!refetch && !invalidateCache) {
          ...
          if (find(equals(providedIndexedParams), cachedParams)) {
