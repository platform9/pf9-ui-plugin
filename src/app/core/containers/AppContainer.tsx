...
import { head, path, pathOr, propEq, isEmpty, prop, isNil } from 'ramda'
...
const AppContainer = () => {
  ...
  const setupSession = async ({ username, unscopedToken, expiresAt, issuedAt }) => {
    ...
    try {
      ...
      showToast(
        `There has been an error initializing the session.\n${err.message}`,
        MessageTypes.error,
      )
      return false
    }
  ...
  const loadingMessage = sessionChecked ? 'Initializing session...' : 'Loading app...'
