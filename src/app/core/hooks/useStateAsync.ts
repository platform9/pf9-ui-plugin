...
type AsyncSetState<T> = (stateUpdate: SetStateAction<T>) => Promise<T>
...
const useStateAsync = <T>(initialState: T): [T, AsyncSetState<T>] => {
  ...
  const setStateAsync = useCallback(
    async (stateUpdater: SetStateAction<T>) => {
      return new Promise<T>((resolve, reject) => {
        setState((prevState) => {
          try {
            const newState =
              stateUpdater instanceof Function ? stateUpdater(prevState) : stateUpdater
