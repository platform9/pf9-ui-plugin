export const notImplementedYet = (req, res) =>
  res.status(500).send('Not implemented yet in simulator.')
...
export const findById = (arr) => (id) =>
  // eslint-disable-next-line eqeqeq
  (typeof arr === 'function' ? arr() : arr).find((x) => x.id == id)
...
export const mapAsJson = (arr, ...params) =>
  ensureArray(arr).map((x) => (x.asJson && x.asJson(...params)) || null)

export const jsonOrNull = (obj) => (obj && obj.asJson && obj.asJson()) || null

export const whitelistKeys = (allowedKeys) => (obj) =>
