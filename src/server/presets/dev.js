...
function loadPreset() {
  ...
  // Tenants
  const serviceTenant = new Tenant({
    id: 'id-tenant-service',
    name: 'service',
  ...
  const tenantParams = { domain_id: '77c53dae6ab6421caaddc34cd56becb4' }
  ...
  // Floating IPs
  new FloatingIp({
    floating_ip_address: '123.234.123',
    subnet_id: 'qwertyuiop',
    port_id: 'asdfasdf',
    project_id: '1234abcd',
    tenant_id: '1234abcd',
    fixed_ip_address: '10.1.10.123',
    description: 'preset floating ip',
    floating_network_id: 'zxcvbnm',
    status: 'ACTIVE',
    router_id: 'awsd',
  })
  ...
  new Volume({
    name: 'TestVolume1',
    description: 'Docker storage test.',
    volume_type: 'sfvol',
    metadata: '',
    size: 15,
    bootable: false,
    status: 'available',
    tenant: 'Dev Team Tenant',
    source: 'Image',
    host: 'host.company.sys',
  ...
  new Volume({
    name: 'TestVolume4',
    description: 'Convert volume to ext.',
    volume_type: 'sfvol',
    metadata: '',
    size: 1,
    bootable: false,
  ...
  new Image({
    name: 'Test-Image-1',
    pf9_description: 'Base CentOS Image Version 5',
    status: 'OK',
    owner: 'Development Team Tenant',
    visibility: 'private',
    protected: false,
    disk_format: 'qcow2',
    virtual_size: 40,
  ...
  new Image({
    name: 'Test-Image-3',
    pf9_description: 'A plain test image.',
    status: 'OK',
    owner: 'Development Team Tenant',
    visibility: 'private',
    protected: true,
  ...
  new SshKey({
    name: 'test1',
    public_key: 'sa9d87hf90sa7d98h7f0a9s87hf8907hasdf',
  })
  new SshKey({
    name: 'test2',
    public_key: '8wekaf098kewf7s9dk7f98k7we98fs908d7f',
  })

  // Hypervisors
  new Hypervisor({
    resMgrId: '1awf13fjsf90j',
    hypervisor_hostname: 'fake hypervisor',
    status: 'enabled',
    host_ip: '1.2.3.4',
    ipInfo: [{ ip: '1.2.3.4', if_name: 'my-interface' }],
  })

  // ResMgrHosts
  const resMgrHost = new ResMgrHost({
    roles: ['pf9-ostackhost'],
    info: { hostname: 'fake resmgr host' },
  })
  const resMgrHost2 = new ResMgrHost({
    roles: ['pf9-ostackhost', 'pf9-kube'],
    info: { hostname: 'fake resmgr host 2' },
  })
  const resMgrHost3 = new ResMgrHost({
    roles: ['pf9-ostackhost', 'pf9-kube'],
    info: { hostname: 'fake resmgr host 3' },
  })

  // Cloud Providers
  CloudProvider.create({
    data: { name: 'mockAwsProvider', type: 'aws' },
    context,
  })
  CloudProvider.create({
    data: { name: 'mockAzureProvier', type: 'azure' },
    context,
  })
  CloudProvider.create({
    data: {
  ...
  // Namespaces
  const defaultNamespace = Namespace.create({
    data: { name: 'default' },
    context,
    config: { clusterId: cluster.uuid },
    raw: true,
  })
  const defaultNamespace2 = Namespace.create({
    data: { name: 'default' },
    context,
    config: { clusterId: cluster2.uuid },
    raw: true,
  })

  // Pods
  Pod.create({
    data: { metadata: { name: 'fakePod' } },
    context,
    config: { clusterId: cluster.uuid, namespace: defaultNamespace.name },
  })
  Pod.create({
    data: { metadata: { name: 'fakePod2' } },
    context,
    config: { clusterId: cluster2.uuid, namespace: defaultNamespace2.name },
  })

  // Deployments (and pods)
  Deployment.create({
    data: {
      metadata: { name: 'fakeDeployment' },
      spec: { replicas: 2 },
    },
    context,
    config: { clusterId: cluster.uuid, namespace: defaultNamespace.name },
  ...
  Logging.create({
    data: loggingsForAllClusters[0],
    context,
    config: { clusterId: cluster.uuid },
  })
  Logging.create({
    data: loggingsForAllClusters[1],
    context,
    config: { clusterId: cluster2.uuid },
  })
  Logging.create({
    data: loggingsForAllClusters[2],
    context,
    config: { clusterId: cluster3.uuid },
  })
  Logging.create({
    data: loggingsForAllClusters[3],
    context,
    config: { clusterId: cluster4.uuid },
  })
  Logging.create({
    data: loggingsForAllClusters[4],
    context,
    config: { clusterId: cluster5.uuid },
  })
}
