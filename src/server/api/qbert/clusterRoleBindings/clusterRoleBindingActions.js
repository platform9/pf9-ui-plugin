...
export const postClusterRoleBinding = (req, res) => {
  ...
  if (
    ...
    return res.status(409).send({
      code: 409,
      message: `clusterRoleBinding ${clusterRoleBinding.metadata.name} already exists`,
    })
  }

  const newClusterRoleBinding = ClusterRoleBinding.create({
...
export const deleteClusterRoleBinding = (req, res) => {
  ...
  const clusterRoleBinding = ClusterRoleBinding.findByName({
