...
export const postService = (req, res) => {
  ...
  if (
    Service.findByName({ name: service.metadata.name, context, config: { clusterId, namespace } })
  ) {
    return res
      .status(409)
      .send({ code: 409, message: `services ${service.metadata.name} already exists` })
  }
...
export const deleteService = (req, res) => {
  ...
  const service = Service.findByName({
    name: serviceName,
    context,
