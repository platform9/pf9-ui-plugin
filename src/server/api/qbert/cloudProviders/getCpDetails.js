...
const randomAwsRegionDetails = (regionId) => {
  return {
    ...
    keyPairs: uniq(
      times(() => ({ KeyName: faker.fake('{{name.firstName}}'), KeyFingerprint: uuid.v4() }), 5),
    ),
...
const randomOpenstackRegionDetails = (regionId) => {
  return {
    ...
    images: times(
      () => ({ id: faker.random.uuid(), name: faker.random.word(), size: faker.random.number }),
      10,
    ),
    keyPairs: times(
      () => ({
  ...
  skus: [
    {
      ...
      capabilities: {
        ...
        ACUs: '100',
        vCPUsPerCore: '1',
        CombinedTempDiskAndCachedIOPS: '1000',
        CombinedTempDiskAndCachedReadBytesPerSecond: '20971520',
    ...
    },
    {
      ...
      capabilities: {
        ...
        ACUs: '100',
        vCPUsPerCore: '1',
        CombinedTempDiskAndCachedIOPS: '2000',
        CombinedTempDiskAndCachedReadBytesPerSecond: '41943040',
        CombinedTempDiskAndCachedWriteBytesPerSecond: '20971520',
        EphemeralOSDiskSupported: 'False',
      },
      family: 'standardAv2Family',
      locationInfo: [{ location: 'westus', zones: ['1', '2', '3'] }],
...
export const getCpRegionDetails = (req, res) => {
  ...
  if (cp.type === 'local') {
    ...
    if (sshKeyMap[cloudProviderId] && sshKeyMap[cloudProviderId][regionId]) {
      awsRegionDetails.keyPairs = [
        ...awsRegionDetails.keyPairs,
        ...sshKeyMap[cloudProviderId][regionId],
      ]
    }
