...
const rowActions = [{ icon: <ReplayIcon />, label: 'Restart', action: onRestart }]
...
addStories('Common Components/ListTable', {
  'Minimal use case': () => <DefaultListTable {...actions} />,

  'w/o checkboxes': () => <DefaultListTable showCheckboxes={false} />,

  'w/ row actions': () => <DefaultListTable rowActions={rowActions} />,

  'w/ columns selection': () => <DefaultListTable canEditColumns rowActions={rowActions} />,

  'w/ columns ordering': () => (
    <DefaultListTable editableColumns draggableColumns rowActions={rowActions} />
  ),

  'w/ filters': () => <DefaultListTable filters={filters} rowActions={rowActions} />,
})
