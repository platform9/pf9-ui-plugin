...
describe('Volumes', async () => {
  ...
  it('create, get and delete a volume placeholder', async () => {
    ...
    await waitUntil({
      condition: waitForVolumeCreate(newVolume.id),
      delay: 1000,
      maxRetries: 20,
    })
    ...
    await waitUntil({
      condition: waitForVolumeDelete(newVolume.id),
      delay: 1000,
      maxRetries: 20,
    })
  ...
  it('create a volumetype placeholder, unset its volumetype tag, then delete it', async () => {
    ...
    const updatedVolumeType = await client.cinder.getVolumeType(
      'UITEST_TestVolumeType'
    )
  ...
  it('Create a volume placeholder, snapshot it and delete both', async () => {
    ...
    await waitUntil({
      condition: waitForVolumeCreate(volume.id),
      delay: 1000,
      maxRetries: 20,
    })
    ...
    await waitUntil({
      condition: waitForSnapshotCreate(snapshot.id),
      delay: 1000,
      maxRetries: 20,
    })
    ...
    await waitUntil({
      condition: waitForSnapshotDelete(snapshot.id),
      delay: 1000,
      maxRetries: 20,
    })
    await client.cinder.deleteVolume(volume.id)
    await waitUntil({
      condition: waitForVolumeDelete(volume.id),
      delay: 1000,
      maxRetries: 20,
    })
    const volumes = await client.cinder.getAllVolumes()
  ...
  it('Create a volume placeholder, snapshot and update metadata, then delete both', async () => {
    ...
    const updatedSnapshotMetadata = await client.cinder.updateSnapshotMetadata(
      snapshot.id,
      {
        TestKey: 'UITEST_TestValue',
      }
  ...
  it('get region default quotas', async () => {
    const quotas = await client.cinder.getDefaultQuotasForRegion(
      client.activeRegion
    )
    expect(quotas).toBeDefined()
  })
  ...
  it('get quotas for region', async () => {
    const projectId = (await client.keystone.getProjects())[0].id
    const quotas = await client.cinder.getQuotasForRegion(
      projectId,
      client.activeRegion
    )
    expect(quotas).toBeDefined()
  ...
  it('set quotas for region', async () => {
    const projectId = (await client.keystone.getProjects())[0].id
    const oldValue = (await client.cinder.getQuotasForRegion(
      projectId,
      client.activeRegion
    )).groups.limit
    ...
    const newQuota = await client.cinder.getQuotasForRegion(
      projectId,
      client.activeRegion
    )
    expect(newQuota.groups.limit).toBe(10)
